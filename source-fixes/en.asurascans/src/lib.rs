...
use aidoku::{
	...
	ImageRequestProvider, PageContext, Home, HomeComponent, HomeComponentValue, HomeLayout, Link, Listing, ListingProvider, Manga,
...
const READER_ORIGINS: &[&str] = &[BASE_URL, "https://api.asurascans.com"];

fn read_html(url: &str) -> Result<aidoku::imports::html::Document> {
    transport::get(url, READER_ORIGINS, HashMap::new())?.get_html()
}
...
impl Source for AsuraScans {
	...
	fn get_page_list(&self, manga: Manga, chapter: Chapter) -> Result<Vec<Page>> {
        // Refresh account state once. Never bypass premium/locked chapter authorization.
        let login = auth::get_login_status().ok();
        if chapter.locked && !login.as_ref().is_some_and(|s| s.is_subscribed) {
            bail!("This chapter is locked. Sign in with an authorized website subscription");
        }
        let mut headers = HashMap::new();
        if let Some(status) = &login {
            headers.insert("Authorization".into(), format!("Bearer {}", status.access_token));
            headers.insert("Cookie".into(), format!("access_token={}; refresh_token={}", status.access_token, status.refresh_token));
        }
        let api_url = format!("{API_URL}/series/{}/chapters/{}", manga.key, chapter.key);
        if let Ok(response) = transport::get(&api_url, READER_ORIGINS, headers.clone())
            && let Ok(json) = response.json::<serde_json::Value>()
            && let Ok(urls) = page_data::urls(&json["data"]["chapter"]["pages"]) {
            return Ok(urls.into_iter().map(|url| Page {content: PageContent::url(url), ..Default::default()}).collect());
        }
        let url = helpers::get_chapter_url(&chapter.key, &manga.key);
        let html = transport::get(&url, READER_ORIGINS, headers)?.get_html()?;
        let props = html.select_first("astro-island[component-url*=ChapterReader], astro-island[opts*=ChapterReader]")
            .and_then(|e| e.attr("props"))
            .ok_or_else(|| error!("Asura did not return a chapter reader. Verify website access/login, then retry"))?;
        let json: serde_json::Value = serde_json::from_str(&props)?;
        let urls = page_data::urls(&json["pages"]).map_err(|e| error!("{e}"))?;
        Ok(urls.into_iter().map(|url| Page {content: PageContent::url(url), ..Default::default()}).collect())
    }
...
impl ListingProvider for AsuraScans {
	fn get_manga_list(&self, listing: Listing, page: i32) -> Result<MangaPageResult> {
		match listing.id.as_str() {
			"Ranking" => {
				let html = read_html(&format!("{BASE_URL}/series-ranking"))?;
				let entries = html
...
impl WebLoginHandler for AsuraScans {
	fn handle_web_login(&self, key: String, cookies: HashMap<String, String>) -> Result<bool> {
        if key == "verifyAccess" { return Ok(cookies.contains_key("cf_clearance")); }
...
impl ImageRequestProvider for AsuraScans {
    fn get_image_request(&self, url: String, _context: Option<PageContext>) -> Result<Request> {
        if !url.starts_with("https://") { bail!("Unsafe image URL"); }
        // CDN requests need the reader origin, never the user's API credentials.
