...
def required_severity(value: dict) -> str:
    kind = value.get("classification", value.get("kind", "unknown"))
    if kind in {"ok", "healthy"}:
        return "healthy"
    if not value.get("conclusive"):
        return "unknown"
    if kind in FAILURES or not value.get("reachable"):
        return "critical"
    return "degraded"
