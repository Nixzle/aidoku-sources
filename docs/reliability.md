...
## Completion repair: client updates and truthful incidents

Aidoku `BrowseViewModel.loadUpdates()` compares the advertised version with the
installed version using `>`. A same-version package replacement was therefore
insufficient delivery. `republishedSources` opts only Read Comics Online into a
monotonic client version, starting at 4. The transform changes only the package
manifest version. Original upstream package/version and WASM hashes are recorded.
Every run checks the live upstream for this opted-in source so a republished cache
cannot masquerade as an unmodified upstream package. Outages preserve the verified
published cache without increasing its version; changed upstream bytes advance the
client version even if the upstream itself repeats a version number.

The runtime job exports actual functional status to its incident reporter.
Protection-blocked, inconclusive, missing-output and failed results keep the incident
open. Only successful workflow execution plus explicit `passed` acceptance closes
it. A previously blocked request followed by a successful response no longer masks
a later parser error as protection. Timeout stdout/stderr and process exit codes are
retained. Candidate regeneration is restricted to pull requests; main and Pages runs
execute committed packages, not a silently rebuilt candidate.

Open owner-specific acceptance: reproduce the Chapter 220 screenshot after its
source, series and installed application version are supplied. Do not label that
unidentified chapter repaired based on another series' smoke result.
